...
//
// This function computes the color at the intersection point
...
void Renderer::Render(const Scene &scene)
{
    ...
    float vstep = -2. / scene.height;
    float vstart = 1. + vstep;
    float hstep = -vstep;
    float hstart = -1. * imageAspectRatio + hstep;
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            x = (hstart + i * hstep) * scale;
            y = (vstart + j * vstep) * scale;
