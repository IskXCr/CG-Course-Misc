...
public:
    ...
    // Return dimension that has the maximum length. 0 - x, 1 - y, 2 - z.
    int maxExtent() const
    {
        Vector3f d = Diagonal();
        if (d.x > d.y && d.x > d.z)
    ...
    // Returns the continuous position of point p relative to the corners of this bounding box. (0, 0, 0) -> (1, 1, 1)
